...
function initCannon() {
  ...
  const box1Shape = new CANNON.Box(new CANNON.Vec3(0.5, 8, 0.5))
  const box1Body = new CANNON.Body({ mass: 0 })
  box1Body.addShape(box1Shape)
  box1Body.position.set(20, 7, -2.5)
  bodies.push(box1Body)
  world.addBody(box1Body)

  const box2Body = new CANNON.Body({ mass: 0 })
  box2Body.addShape(box1Shape)
  box2Body.position.set(20, 7, 2.5)
  bodies.push(box2Body)
  world.addBody(box2Body)

  const box3Shape = new CANNON.Box(new CANNON.Vec3(0.5, 0.5, 2.5))
  const box3Body = new CANNON.Body({ mass: 0 })
  box3Body.addShape(box3Shape)
  box3Body.position.set(20, 4.5, 0)
  bodies.push(box3Body)
  world.addBody(box3Body)

  const trigerboxShape = new CANNON.Box(new CANNON.Vec3(0.5, 5, 2))
  const myTriggerBody = new CANNON.Body({ isTrigger: true })
  myTriggerBody.addShape(trigerboxShape)
  myTriggerBody.position.set(20, 10, 0)
  bodies.push(myTriggerBody)
  world.addBody(myTriggerBody)

  // It is possible to run code on the exit/enter
  // of the trigger.
  // myTriggerBody.addEventListener('collide', (event) => {
  ...
  world.addEventListener('endContact', (event) => {
    if (
      (event.bodyA === sphereBody && event.bodyB === myTriggerBody) ||
      (event.bodyB === sphereBody && event.bodyA === myTriggerBody)
    ) {
